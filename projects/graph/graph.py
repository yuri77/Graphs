...
class Graph:
    ...
    def bft(self, starting_vertex):
        ...
        # while the queue is not empty, continue looping
        while q.size() > 0:
            v = q.dequeue()
        ...
        for x in self.get_neighbors(starting_vertex):
            if x not in visited:
                new_path = self.dfs_recursive(
                    starting_vertex=x, destination_vertex=destination_vertex, path=path, visited=visited)
